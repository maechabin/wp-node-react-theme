...
const Pagination = (props) => {
  ...
  const pagination = pager.map((page, i) => {
    ...
    return <li key={page + number}><Link to={`${page + number}`}>{page + number}</Link></li>;
  });
  const prev = () => {
    if (paramsPage === 1) {
      return <li>前へ</li>;
    ...
    if (paramsPage > totalPages - 5) {
      return <li><Link to={`${totalPages - 5}`}>前へ</Link></li>;
    }
    return <li><Link to={`${paramsPage - 1}`}>前へ</Link></li>;
  };
  const next = () => {
    if (paramsPage >= totalPages) {
      return <li>次へ</li>;
    }
    return <li><Link to={`${paramsPage + 1}`}>次へ</Link></li>;
  };
